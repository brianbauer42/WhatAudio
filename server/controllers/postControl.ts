...
export const create = (req: Request, res: Response) => {
  if (req.files) {
    ...
    newPost.audioUri =
      req.user.displayName + "/" + path.basename(req.files.audio.path);
    if (req.files.art) {
      newPost.artUri =
        req.user.displayName + "/" + path.basename(req.files.art.path);
    }
...
export const search = (req: Request, res: Response) => {
  ...
  PostModel.find({ $text: { $search: queryString } })
    .populate("sharedBy", "displayName")
    .exec((err: Error, result: IPostModel) => {
      if (err) {
        return res.send(err);
      }
      return res.send(result);
...
export const update = (req: Request, res: Response) => {
  ...
  PostModel.findByIdAndUpdate(
    req.params.id,
    req.body,
    (err: Error, result: IPostModel | null) => {
      if (err) {
        return res.send(err);
      }
      return res.send(result);
